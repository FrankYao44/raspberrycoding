...
class Sentinel(object):
    ...
    def __init__(self, loop, ):
        conn = mysql_database.connect(user=configs['db']['user'], password=configs['db']['password'],
                                      database='MCV_database', auth_plugin='mysql_native_password')
        cursor = conn.cursor()
        self.cursor = cursor
        self.conn = conn
        self.loop = loop
...
#       self.conn.commit()
    ...
    async def __create__(self):
        ...
        cyber_kernel = Cloudpiercer(self.loop)
