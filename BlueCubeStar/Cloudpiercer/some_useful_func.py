...
def password(_time):
	...
	sha1 = hashlib.sha1()
	sha2 = hashlib.sha1()
	sha1.update(str(s-1).encode('utf-8') + configs['safety']['salt'])
	sha2.update(str(s).encode('utf-8'))
...
def introspection(fn):
	...
	fn.args = arg
	fn.kwargs = kw
	fn.position_or_keyword_input = pk_input
	fn.position_or_keyword_default = pk_default
	return wrapper
